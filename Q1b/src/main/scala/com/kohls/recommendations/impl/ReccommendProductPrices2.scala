...
class ReccommendProductPrices2(args: Args) extends PipeCommon(args) {
  ...
  def processPrice(products: Pipe, catRecmnds: Pipe) = {
    ...
    val prodPrcGrpedPipe = prodPrcPids.groupBy(ProductCommon.Prod_Id) { _.sortBy(ProductCommon.price).take(6) }// do not need reverse, need ascending
    if (debug) prodPrcGrpedPipe.write(Tsv(RPaths.prodPrcGrpedFile))
    //need to refer to productId here so we can filter it out from recommendations during fold.
    val prodFold = prodPrcGrpedPipe.groupBy('productId) {
