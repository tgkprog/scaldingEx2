...
class Recmd1(args: Args) extends Job(args) {
  ...
  def process(products1: Pipe, catRecmnds: Pipe) = {
    ...
    }.map(('primaryType_t1, 'type_t2, 'subType_t3) -> ('catFul)) {
      x: (String, String, String) =>
        x._1.concat("_").concat(x._2).concat("_").concat(x._3)
    }
    val cats = products.unique('primaryType_t1, 'type_t2, 'subType_t3) //TODO .fitler('gender =)
    println("cats made 2");
  ...
  def processQ1Type1Match(jointProdWithRecoCat: Pipe, catRecmnds: Pipe) = {
    ...
    val n2 = n.project('productId, 'brand, 'style, 'gender, 'primaryType_t1, 'type_t2, 'subType_t3, 'MyRecomdProducts);
